...
// ── local assets ──────────────────────────────────────────────────────────────
import bg1    from '../assets/home/slide-1.jpg';
import bg2    from '../assets/home/hero-text.webp';
import build1 from '../assets/home/hero-text.webp';
import build2 from '../assets/home/slide-2.jpg';
import t1     from '../assets/home/1.webp';
import t2     from '../assets/home/2.webp';
import t3     from '../assets/home/3.webp';
import t4     from '../assets/home/4.webp';
...
const SLIDES = [
  { bg: bg1, building: build1 },
  { bg: bg2, building: build2 },
];

const TEAM = [t1, t2, t3, t4];

// ── Envato Market SVG Logo ────────────────────────────────────────────────────
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    tl.to(
      ...
      { y: '0%', duration: 1, stagger: 0.18, ease: 'power4.out' },
      '-=1.4'
    );

    tl.to(
      buildingRef.current,
      { opacity: 1, scale: 1, rotation: 0, duration: 0.8, ease: 'back.out(1.4)' },
      '-=0.5'
    );

    tl.to(
      [teamRef.current, scrollRef.current],
      { opacity: 1, y: 0, duration: 0.7, stagger: 0.12, ease: 'power3.out' },
      '-=0.4'
    );

    tl.from(counterRef.current, { opacity: 0, x: 16, duration: 0.5 }, '-=0.5');
  ...
  useEffect(() => {
    const st = ScrollTrigger.create({
      ...
      scrub: true,
      onUpdate: (s) => {
        if (bgRef.current) gsap.set(bgRef.current, { yPercent: s.progress * 22 });
      },
    });
    return () => st.kill();
  }, []);
  ...
  const switchSlide = (i) => {
    ...
    gsap.to(bgRef.current, {
      ...
      onComplete: () => {
        setActiveSlide(i);
        gsap.to(bgRef.current, { opacity: 1, duration: 0.55 });
      },
    });
  };
  ...
  return (
    <section className="hero" id="hero">
      ...
      <div className="hero-bg">
        <img ref={bgRef} src={SLIDES[activeSlide].bg} alt="" className="hero-bg-img" />
      </div>
      ...
      {/* Slide dots — right edge, vertically centred */}
      ...
      <div className="hero-content">
        <h1 className="hero-headline">
          ...
          <span className="h-line">
            <span className="h-inner" ref={line1Ref}>
              Innovation in Every Line,
            </span>
          </span>

          <span className="h-line">
            <span className="h-inner h-row" ref={line2Ref}>
              <span>Strength</span>
              <span className="hero-oval" ref={buildingRef}>
                <img src={SLIDES[activeSlide].building} alt="architecture" />
              </span>
              <span>in</span>
            </span>
          </span>
          ...
          <span className="h-line">
            <span className="h-inner" ref={line3Ref}>
              Every Build
            </span>
          </span>
      ...
      <div className="hero-bottom">
        ...
        <div className="hero-team" ref={teamRef}>
          <div className="hero-avatars">
            {TEAM.map((src, i) => (
              <span className="h-avatar" key={i}>
                <img src={src} alt={`Team ${i + 1}`} />
              </span>
            ))}
            <span className="h-avatar h-avatar-more">
              <i className="fas fa-ellipsis" />
            </span>
          </div>
          <p className="hero-team-label">Meet Our Team</p>
        </div>

        <div className="hero-scroll" ref={scrollRef}>
          <span className="hero-scroll-txt">Scroll For More</span>
