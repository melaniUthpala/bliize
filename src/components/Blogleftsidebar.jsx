...
export default function BlogLeftSidebar() {
  ...
  return (
    <article className="brs-wrap">
      ...
      <div className="brs-layout brs-layout-left">
        {/* Sidebar on the LEFT */}
        <aside className="brs-side">
          <div className="brs-card">
            <h3 className="brs-side-title">Search here</h3>
            <form className="brs-search" onSubmit={(e) => e.preventDefault()}>
              <input type="text" placeholder="Search Post.." />
              <button type="submit" aria-label="Search">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3" strokeLinecap="round"/></svg>
              </button>
            </form>
          </div>

          <div className="brs-card">
            <h3 className="brs-side-title">Categories</h3>
            <ul className="brs-cats">
              {CATEGORIES.map((c, i) => (
                <li key={i}><a href="#"><span>{c.name}</span><span>{c.count}</span></a></li>
              ))}
            </ul>
          </div>

          <div className="brs-card">
            <h3 className="brs-side-title">Related Posts</h3>
            <ul className="brs-related">
              {POSTS.map((p, i) => (
                <li key={i}>
          ...
          <div className="brs-card">
            <h3 className="brs-side-title">Populer Tags</h3>
            <div className="brs-tags">
              {TAGS.map((t, i) => (
                <a key={i} href="#" className="brs-tag">{t}</a>
              ))}
            </div>
          </div>
        </aside>
